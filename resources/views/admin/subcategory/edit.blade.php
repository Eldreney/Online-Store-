...
<section class="content">
    <div class="container-fluid">
        ...
        <form action="{{ route('subcategory.update', $subcategory->id) }}" method="POST" id="subCategoryForm">
            @csrf
            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category_id" id="category_id" class="form-control">
                                    ...
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}"
                                            {{ (int)$subcategory->category_id === (int)$cat->id ? 'selected' : '' }}>
                                            {{ $cat->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="status" id="status" class="form-control">
                                    <option value="1" {{ (int)$subcategory->status === 1 ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ (int)$subcategory->status === 0 ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
